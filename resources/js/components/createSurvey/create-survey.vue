<template>
    <div
        class="flex flex-col justify-center w-full mx-auto mt-8 mb-8 space-y-4"
    >
...
function swapElements(index) {
    ...
    list.value[swapElementFirst.value.nb] =
        list.value[swapElementSecond.value.nb];
...
function deleteElement(id_arg) {
    list.value = list.value.filter(
        (elementOfTheList) => elementOfTheList.id !== id_arg
    );
}
